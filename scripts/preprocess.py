...
def main(input_dir,output_dir):
    ...
    for image_path in tqdm(image_files, desc=f"Processing images in {input_dir}"):
        try:
            ...
            cropped = processor.crop_image(half_width=settings.crop_width,left_offset=settings.left_offset,right_offset=settings.right_offset)
            
            # Step 2: Apply bilateral filtering
            bilateral = processor.apply_bilateral_filter(d=settings.diameter_neighbor_pixel, sigmaColor=settings.sigma_color, sigmaSpace=settings.sigma_space)
            
            # Step 3: Enhance contrast using CLAHE
            contrast = processor.apply_clahe(clipLimit=settings.clip_limit, tileGridSize=settings.tile_grid_size)
            
            # Step 4: Sharpen the image (using Laplacian-based sharpening)
            sharp = processor.sharpen_image(weight=settings.weight)
            
            # Step 5: Apply multi-scale Sobel edge detection
            edges = processor.apply_sobel_edge(scales=settings.scales)
        
            # Step 6: Apply morphological closing
            skeleton = processor.apply_morph_skeletonize(kernel=settings.morph_kernel)
